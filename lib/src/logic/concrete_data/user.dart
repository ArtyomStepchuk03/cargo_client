...
class Role {
  ...
  static bool isLogisticianOrHigher(String? role) =>
      role == logistician || role == administrator;
  static bool isManagerOrHigher(String? role) =>
      role == manager || isLogisticianOrHigher(role);
  static bool isDispatcherOrHigher(String? role) =>
      role == dispatcher || isLogisticianOrHigher(role);
}
...
extension UserPermissions on User {
  bool canAddOrders() {
    ...
    if (role == Role.dispatcher) {
      return carrier?.orderPermissions?.create == true;
    }
  ...
  bool canAddUnloadingPoints() =>
      Role.isManagerOrHigher(role) || role == Role.customer;
  bool canAddUnloadingEntrances() => Role.isManagerOrHigher(role);
  bool canAddUnloadingContacts() =>
      Role.isManagerOrHigher(role) || role == Role.customer;
  bool canAddTransportUnits() =>
      Role.isManagerOrHigher(role) || Role.isDispatcherOrHigher(role);
