...
class VehicleEquipment {
  ...
  static VehicleEquipment? decode(Map<String, dynamic>? data) {
...
class VehicleAdmissionData {
  ...
  static VehicleAdmissionData? decode(Map<String, dynamic>? data) {
    if (data == null) return null;
    return VehicleAdmissionData(
      inspectionEndDate: data['inspectionEndDate'] != null
          ? DateTime.tryParse(data['inspectionEndDate'])
          : null,
      mtplEndDate: data['mtplEndDate'] != null
          ? DateTime.tryParse(data['mtplEndDate'])
          : null,
    );
  }
}
...
class VehiclePass {
  ...
  static VehiclePass? decode(Map<String, dynamic>? data) {
    if (data == null) return null;
    return VehiclePass(
        city: data['city'],
        zone: validateEnumeration(data['zone'], VehiclePassMoscowZone.values),
        timeOfAction: enumerationFromString(
            data['timeOfAction'], VehiclePassTimeOfAction.values),
        beginDate: data['beginDate'] != null
            ? DateTime.tryParse(data['beginDate'])
            : null,
        endDate:
...
class Vehicle extends Identifiable<String> {
  ...
  static Vehicle? decode(Decoder decoder) {
    ...
    decoded.admissionData =
        VehicleAdmissionData.decode(decoder.decodeMap('admissionData'));
    decoded.passes =
        decoder.decodeMapList('passes', (data) => VehiclePass.decode(data));
