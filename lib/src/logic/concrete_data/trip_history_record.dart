...
class TripHistoryAdditionalData {
  ...
  static TripHistoryAdditionalData? decode(Map<String, dynamic>? data) {
...
class TripHistoryRecord extends Identifiable<String> {
  ...
  static TripHistoryRecord? decode(Decoder decoder) {
    if (!decoder.isValid()) {
      return null;
    }
    ...
    decoded.additionalData =
        TripHistoryAdditionalData.decode(decoder.decodeMap('additionalData'));
    decoded.ignoreWrongCoordinate =
        decoder.decodeBooleanDefault('ignoreWrongCoordinate');
    return decoded;
  }
}
