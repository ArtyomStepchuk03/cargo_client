...
class Trip extends Identifiable<String> {
  ...
  static Trip? decode(Decoder decoder) {
    if (!decoder.isValid()) {
      return null;
    }
    final decoded = Trip();
    ...
    decoded.historyRecords = decoder.decodeObjectList('historyRecords',
        (Decoder decoder) => TripHistoryRecord.decode(decoder));
    decoded.problems = decoder.decodeObjectList(
        'problems', (Decoder decoder) => TripProblem.decode(decoder));

    if (decoded.historyRecords != null) {
      if (decoded.historyRecords!.every((record) => record?.stage != null)) {
        decoded.historyRecords!.sort(
            (one, other) => one!.stage!.index.compareTo(other!.stage!.index));
      }
    }
