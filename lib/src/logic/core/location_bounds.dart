...
extension LatLngBoundsUtility on LatLngBounds {
  LatLng get center {
    final latitude =
        southwest.latitude + (northeast.latitude - southwest.latitude) / 2;
    final longitude =
        southwest.longitude + (northeast.longitude - southwest.longitude) / 2;
    return LatLng(latitude, longitude);
  }

  LatLngSpan get span {
    return LatLngSpan(northeast.latitude - southwest.latitude,
        northeast.longitude - southwest.longitude);
  }

  static LatLngBounds fromCenterAndSpan(LatLng center, LatLngSpan span) {
    final southwest = LatLng(center.latitude - span.latitudeSpan / 2,
        center.longitude - span.longitudeSpan / 2);
    final northeast = LatLng(center.latitude + span.latitudeSpan / 2,
        center.longitude + span.longitudeSpan / 2);
