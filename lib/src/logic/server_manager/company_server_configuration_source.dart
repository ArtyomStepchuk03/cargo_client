...
class CompanyServerConfigurationSource {
  Future<List<CompanyServerInformation>> get() async {
    if (kDebugMode) {
      return [
        CompanyServerInformation(
            name: "Develop",
            configuration: DefaultServerConfiguration.makeDevelop())
      ];
    }
    final headers = {'Content-Type': http_utility.contentTypeJson};
    final response = await http.get(
        Uri.parse('https://config.cargodeal.ru/companies.json'),
        headers: headers);
  ...
  CompanyServerInformation? _decodeServerInformation(
