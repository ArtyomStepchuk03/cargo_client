...
class OrderProgressNotifier {
  ...
  OrderProgressNotifier(
      this.orderServerAPI, this.offerServerAPI, this.tripServerAPI)
      : _offerSubscriptions = {},
        _tripSubscriptions = {};
  ...
  void unsubscribe() {
    ...
    for (final subscription in _offerSubscriptions.values)
      offerServerAPI.unsubscribe(subscription);
    for (final subscription in _tripSubscriptions.values)
      tripServerAPI.unsubscribe(subscription);
  ...
  void _updateTripSubscriptions(Order order) {
    var tripsToSubscribe = order.offers
            ?.map((offer) => offer!.trip)
