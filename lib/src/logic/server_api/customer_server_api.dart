...
class CustomerServerAPI implements SkipPagedDataSource<Customer> {
  ...
  Future<List<Customer>> list(int skip, int limit,
      {Manager? manager, bool includeDeleted = false}) async {
    final builder = parse.QueryBuilder(Customer.className);
    if (!includeDeleted) {
      builder.equalTo('deleted', false);
    }
    if (manager != null) {
      final unloadingPointBuilder =
          parse.QueryBuilder(UnloadingPoint.className);
      unloadingPointBuilder.equalToObject(
          'manager', Manager.className, manager.id);
    ...
    return results
        .map((json) => Customer.decode(Decoder(json)))
        .whereType<Customer>()
        .toList();
  }
  ...
  Future<void> fetchUnloadingPoints(Customer customer) async {
    final fetched = await parse.getById(
        serverManager.server!, Customer.className, customer.id!,
        include: ['unloadingPoints', 'unloadingPoints.manager']);
    if (fetched == null) {
      throw RequestFailedException();
    }
    customer.unloadingPoints = Decoder(fetched)
        .decodeObjectList('unloadingPoints',
            (Decoder decoder) => UnloadingPoint.decode(decoder))!
        .excludeDeleted();
  }
  ...
  Future<void> create(Customer customer) async {
    ...
    final id =
        await parse.create(serverManager.server!, Customer.className, data);
    customer.id = id;
    customer.internal = true;
    customer.deleted = false;
  }

  Future<void> addUnloadingPoint(Customer? customer,
      UnloadingPoint unloadingPoint, Manager? manager) async {
    final unloadingPointData = <String, dynamic>{};
    final unloadingPointEncoder = Encoder(unloadingPointData);
    if (manager != null) {
      unloadingPointEncoder.encodePointer(
          'manager', Manager.className, manager.id);
    }
    unloadingPoint.encode(unloadingPointEncoder);

    final id = await parse.create(
        serverManager.server!, UnloadingPoint.className, unloadingPointData);
    unloadingPoint.id = id;

    final customerData = <String, dynamic>{};
    final customerEncoder = Encoder(customerData);
    ListEncoder listEncoder =
        customerEncoder.getAddOperationListEncoder('unloadingPoints');
    listEncoder.addPointer(UnloadingPoint.className, unloadingPoint.id!);

    await parse.update(
        serverManager.server!, Customer.className, customer?.id, customerData);
  ...
  Future<ContractorVerifyResult> verify(Customer? customer) async {
    ...
    final resultData = await callCloudFunction(
        serverManager.server!, 'Dispatcher_verifyCustomer', parameters);
  ...
  Future<ContractorVerifyResult> getStatus() async {
    final resultData =
        await callCloudFunction(serverManager.server!, 'Customer_getStatus');
    final result = ContractorVerifyResultDecode.decode(resultData);
    if (result == null) {
      throw InvalidResponseException();
    }
