...
class SupplierServerAPI implements SkipPagedDataSource<Supplier> {
  ...
  Future<List<Supplier>> list(int skip, int limit,
      {bool? transfer, bool includeDeleted = false}) async {
    ...
    return results
        .map((json) => Supplier.decode(Decoder(json)))
        .whereType<Supplier>()
        .toList();
  }
  ...
  Future<void> fetchArticleBrands(Supplier supplier) async {
    final fetched = await parse.getById(
        serverManager.server!, Supplier.className, supplier.id!,
        include: ['articleBrands.type']);
    if (fetched == null) {
      throw RequestFailedException();
    }
    supplier.articleBrands = Decoder(fetched)
        .decodeObjectList(
  ...
  Future<void> fetchLoadingPoints(Supplier supplier) async {
    final fetched = await parse.getById(
        serverManager.server!, Supplier.className, supplier.id!,
        include: ['loadingPoints']);
    if (fetched == null) {
      throw RequestFailedException();
    }
    supplier.loadingPoints = Decoder(fetched)
        .decodeObjectList(
  ...
  Future<ContractorVerifyResult> verify(Supplier? supplier) async {
    ...
    final resultData = await callCloudFunction(
        serverManager.server!, 'Dispatcher_verifySupplier', parameters);
