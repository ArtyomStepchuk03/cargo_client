...
class UserServerAPI {
  ...
  Future<User?> getById(String? id) async {
    final result = await parse.getUserById(serverManager?.server, id, include: [
      'logistician',
      'manager',
      'dispatcher',
      'customer',
  ...
  Future<List<User>> list(List<String> roles, int skip, int limit) async {
    ...
    return results
        .map((json) => User.decode(Decoder(json)))
        .whereType<User>()
        .toList();
  }

  Future<User?> logIn(String userName, String password) async {
    final body = await serverManager?.server?.performGet(
        'login?username=${Uri.encodeQueryComponent(userName)}&password=${Uri.encodeQueryComponent(password)}',
