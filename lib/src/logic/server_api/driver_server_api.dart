...
class DriverServerAPI {
  ...
  Future<List<Driver>> listFree(int skip, int limit, Carrier carrier) async {
    ...
    return results
        .map((json) => Driver.decode(Decoder(json)))
        .whereType<Driver>()
        .toList();
  }
  ...
  Future<void> create(Driver driver) async {
    ...
    final id =
        await parse.create(serverManager.server!, Driver.className, data);
    driver.id = id;
  ...
  Future<bool> verify(Driver? driver) async {
    ...
    final result = await callCloudFunction(
        serverManager.server!, 'Dispatcher_verifyDriver', parameters);
