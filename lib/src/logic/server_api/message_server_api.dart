...
class MessageRecipients {
  ...
  bool operator ==(dynamic other) {
    ...
    return all == otherMessageRecipients.all &&
        role == otherMessageRecipients.role &&
        user == otherMessageRecipients.user;
  }
...
class MessageServerAPI implements SkipPagedDataSource<Message> {
  ...
  Future<List<Message>> list(int skip, int limit) async {
    ...
    return results
        .map((json) => Message.decode(Decoder(json)))
        .whereType<Message>()
        .toList();
  }
  ...
  Future<void> send(
      MessageRecipients? recipients, String? title, String? body) async {
    final parameters = {
    ...
    await callCloudFunction(
        serverManager.server!, 'Dispatcher_sendMessage', parameters);
  }
