...
class CarrierServerAPI implements SkipPagedDataSource<Carrier> {
  ...
  Future<List<Carrier>> list(int skip, int limit) async {
    ...
    return results
        .map((json) => Carrier.decode(Decoder(json)))
        .whereType<Carrier>()
        .toList();
  }
  ...
  Future<void> fetchSuppliers(Carrier carrier) async {
    final fetched = await parse.getById(
        serverManager.server!, Carrier.className, carrier.id!,
        include: ['suppliers']);
    if (fetched == null) {
      throw RequestFailedException();
    }
    carrier.suppliers = Decoder(fetched)
        .decodeObjectList(
  ...
  Future<void> fetchCustomers(Carrier? carrier) async {
    ...
    carrier?.customers = Decoder(fetched)
        .decodeObjectList(
