...
class CombinedDataSource<T> implements DataSource<T> {
  ...
  Future<DataPortion<T>> loadPortion(dynamic token, int suggestedLimit) async {
    ...
    final portion = await dataSources[dataSourceIndex]
        .loadPortion(internalToken?.innerToken, suggestedLimit);
    if (portion.finished == true) {
      if (dataSourceIndex + 1 >= dataSources.length) {
        return DataPortion.finished(portion.items);
      }
      return DataPortion(portion.items,
          nextPortionToken:
              CombinedDataPortionToken(dataSourceIndex + 1, null));
    }
    return DataPortion(portion.items,
        nextPortionToken: CombinedDataPortionToken(
            dataSourceIndex, portion.nextPortionToken));
  }
