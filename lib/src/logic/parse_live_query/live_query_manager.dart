...
class LiveQueryManager {
  factory LiveQueryManager(LiveQueryConfiguration configuration) {
    ...
    final subscriptionManager =
        LiveQuerySubscriptionManager(server, connection);
  ...
  LiveQuerySubscription<T?> subscribe<T>(
      ...
      _subscriptionManager.subscribe(query, initializer);
  void unsubscribe(OpaqueLiveQuerySubscription? subscription) =>
      _subscriptionManager.unsubscribe(subscription);

  LiveQuerySubscription<T?> subscribeToObjectChanges<T>(
      String className, String? id, ObjectDecodeInitializer<T?>? initializer) {
  ...
  LiveQueryManager._internal(
      this._server, this._connection, this._subscriptionManager);
}
