...
class LocationTree<T extends LocationTreeItem> {
  ...
  static List<T> _itemsInNode<T extends LocationTreeItem>(
      LatLngBounds requestedBounds,
      LocationTreeNode<T>? node,
      LatLngBounds nodeBounds) {
    if (node?.items != null) {
      return node!.items!;
    }
    ...
    final southwestBounds =
        LatLngBounds(southwest: nodeBounds.southwest, northeast: center);
    final northwestBounds =
        LatLngBounds(southwest: westCenter, northeast: northCenter);
    final northeastBounds =
        LatLngBounds(southwest: center, northeast: nodeBounds.northeast);
    final southeastBounds =
        LatLngBounds(southwest: southCenter, northeast: eastCenter);

    var items = <T>[];

    if (southwestBounds.intersects(requestedBounds)) {
      items.addAll(
          _itemsInNode(requestedBounds, node?.southwest, southwestBounds));
    }
    if (northwestBounds.intersects(requestedBounds)) {
      items.addAll(
          _itemsInNode(requestedBounds, node?.northwest, northwestBounds));
    }
    if (northeastBounds.intersects(requestedBounds)) {
      items.addAll(
          _itemsInNode(requestedBounds, node?.northeast, northeastBounds));
    }
    if (southeastBounds.intersects(requestedBounds)) {
      items.addAll(
          _itemsInNode(requestedBounds, node?.southeast, southeastBounds));
    }
