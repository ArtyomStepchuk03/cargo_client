...
DataSource<Order> makeOrderDataSource(
    {OrderServerAPI? serverAPI,
    User? user,
    OrderSort? sort,
    OrderFilter? filter}) {
  if (filter?.deleted == false && sort!.sortType == OrderSortType.byDefault) {
    if (filter?.progress == null) {
      return CombinedDataSource([
        _PlainOrderDataSource(
            serverAPI: serverAPI,
            user: user,
            filter: OrderFilter.notFullyDistributed(OrderOfferingStatus.none),
            sort: sort),
        _PlainOrderDataSource(
            serverAPI: serverAPI,
            user: user,
            filter: OrderFilter.notFullyDistributed(
                OrderOfferingStatus.carriersOnly),
            sort: sort),
        _PlainOrderDataSource(
            serverAPI: serverAPI,
            user: user,
            filter:
                OrderFilter.notFullyDistributed(OrderOfferingStatus.drivers),
            sort: sort),
        _PlainOrderDataSource(
            serverAPI: serverAPI,
            user: user,
            filter: OrderFilter.notFullyFinished(),
            sort: sort),
        _PlainOrderDataSource(
            serverAPI: serverAPI,
            user: user,
            filter: OrderFilter.fullyFinished(),
            sort: sort),
      ]);
    }
  ...
  return _PlainOrderDataSource(
      serverAPI: serverAPI, user: user, filter: filter, sort: sort);
}
...
class _PlainOrderDataSource implements DataSource<Order> {
  ...
  Future<DataPortion<Order>> loadPortion(
      dynamic token, int suggestedLimit) async {
    DateTime? sortDate = token;
    final items = await serverAPI!
        .list(user, sortDate, suggestedLimit, filter: filter, sort: sort);
    if (items.length < suggestedLimit) {
      return DataPortion.finished(items);
    }

    DateTime? nextToken = this.sort?.sortType == OrderSortType.byDefault
        ? items.last?.lastProgressDate
        : items.last?.unloadingBeginDate;
