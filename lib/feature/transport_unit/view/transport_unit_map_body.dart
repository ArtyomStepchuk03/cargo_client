...
import 'package:manager_mobile_client/util/map_utility/map_cluster/geohash_map_cluster.dart';
...
class TransportUnitMapBodyState extends State<TransportUnitMapBody> {
  void select(TransportUnit? transportUnit, {bool centerInView = false}) {
    ...
    if (centerInView) {
      ...
      _mapController?.moveCamera(
          CameraUpdate.newLatLngZoom(_selectedValue!.coordinate!, zoom));
      _lastZoom = zoom;
    }
    _updateMarkers();
  }
  ...
  List<GeohashMapClusterItem<Tuple2<Supplier, LoadingPoint>>>?
      _loadingPointItems;
  double? _lastZoom;
  ...
  Marker _buildTransportUnitClusterMarker(MapCluster<TransportUnit?> cluster) {
    ...
    return Marker(
      ...
      icon: _imageList.transportUnitClusterIcons!
          .iconForCount(cluster.values.length)!,
      onTap: () => _onClusterMarkerTap(
          cluster.values.map((transportUnit) => transportUnit!.coordinate!)),
    );
  }
  ...
  Marker _buildSingleTransportUnitMarker(TransportUnit? transportUnit) {
    return Marker(
      ...
      icon: _selectedValue != null && transportUnit == _selectedValue
          ? _imageList.transportUnitSelectedIcon!
          : _imageList.transportUnitIcon!,
      infoWindow: InfoWindow(
        title: short.formatDriverSafe(context, transportUnit.driver),
  ...
  String? _buildTransportUnitSnippet(TransportUnit transportUnit) {
    ...
    final status = formatTransportUnitStatus(context, transportUnit.status);
  ...
  Marker _buildLoadingPointClusterMarker(
      MapCluster<Tuple2<Supplier, LoadingPoint>> cluster) {
    if (cluster.values.length == 1) {
      return _buildSingleLoadingPointMarker(
          cluster.values[0].item1, cluster.values[0].item2);
    }
    return Marker(
      markerId: MarkerId('cluster_${cluster.values[0].item2.id}'),
      position: cluster.coordinate,
      icon: _imageList.loadingPointClusterIcons!
          .iconForCount(cluster.values.length)!,
      onTap: () => _onClusterMarkerTap(cluster.values
          .map((value) => value.item2.entrances![0]!.coordinate!)),
    );
  }

  Marker _buildSingleLoadingPointMarker(
      Supplier supplier, LoadingPoint loadingPoint) {
    return Marker(
      markerId: MarkerId(loadingPoint.id!),
      position: loadingPoint.entrances![0]!.coordinate!,
      icon: _imageList.loadingPointIcon!,
      infoWindow: InfoWindow(
        title: short.formatSupplierSafe(context, supplier),
        snippet: short.formatLoadingPointSafe(context, loadingPoint),
      ),
    );
  }
  ...
  void _updateMarkersIfNeeded() {
    if (_lastUpdateZoom != null &&
        !shouldRebuildClusters(_lastUpdateZoom!, _lastZoom!)) {
      return;
    }
    _updateMarkers();
  }
  ...
  void _updateMarkers() {
    ...
    _markers = <Marker>{};
    _markers
        .addAll(transportUnitClusters!.map(_buildTransportUnitClusterMarker));
  ...
  Future<void> _loadData() async {
    final transportUnits = await _transportUnitServerAPI!
        .listForMap(widget.status, widget.carrier);
    final suppliers = await _supplierServerAPI!.listForMap();
    _transportUnitItems = transportUnits
        .map((transportUnit) => GeohashMapClusterItem.encode(
            transportUnit!.coordinate!, transportUnit))
        .toList();
    _loadingPointItems = _expandSuppliers(suppliers)
        as List<GeohashMapClusterItem<Tuple2<Supplier, LoadingPoint>>>?;
  }
    ...
    for (final supplier in suppliers) {
      ...
      for (final loadingPoint in supplier.loadingPoints!) {
        if (loadingPoint?.entrances == null ||
            loadingPoint!.entrances!.isEmpty) {
          continue;
        }
        ...
        items.add(GeohashMapClusterItem.encode(
            entrance!.coordinate!, Tuple2(supplier, loadingPoint)));
      }
    }
    return items;
  }
