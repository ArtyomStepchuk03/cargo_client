...
import 'package:manager_mobile_client/src/logic/parse/objects/objects.dart' as parse;
...
class Decoder {
  ...
  DateTime decodeCreatedAt() => parse.dateFromString(safeCast<String>(_data[parse.createdAtKey]));
  ...
  DateTime decodeDate(String key) => parse.dateFromJson(safeCast<Map<String, dynamic>>(_data[key]));
  LatLng decodeCoordinate(String key) => parse.coordinateFromJson(safeCast<Map<String, dynamic>>(_data[key]));

  Map<String, dynamic> decodeMap(String key) => safeCast<Map<String, dynamic>>(_data[key]);
  RemoteFile decodeFile(String key) => parse.fileFromJson(safeCast<Map<String, dynamic>>(_data[key]));
...
class ListDecoder {
  ...
  DateTime decodeDate(int index) => parse.dateFromJson(safeCast<Map<String, dynamic>>(_data[index]));
  LatLng decodeCoordinate(int index) => parse.coordinateFromJson(safeCast<Map<String, dynamic>>(_data[index]));

  Map<String, dynamic> decodeMap(int index) => safeCast<Map<String, dynamic>>(_data[index]);
  RemoteFile decodeFile(int index) => parse.fileFromJson(safeCast<Map<String, dynamic>>(_data[index]));
...
T enumerationFromString<T>(String stringValue, List<T> values) {
  ...
  return values.firstWhere((value) => value.toString().toLowerCase().split('.').last == stringValue.toLowerCase(), orElse: () => null);
}
...
extension DecoderUtility on Decoder {
  ...
  T decodeEnumeration<T>(String key, List<T> values) {
    final index = decodeNumber(key);
    return validateEnumeration(index, values);
  }

  List<T> decodeMapList<T>(String key, MapDecodeInitializer<T> initializer) => _decodeList(key, (listDecoder, index) => initializer(listDecoder.decodeMap(index)));
  List<T> decodeObjectList<T>(String key, ObjectDecodeInitializer<T> initializer) => _decodeList(key, (listDecoder, index) => initializer(listDecoder.getDecoder(index)));
  ...
  List<T> _decodeList<T>(String key, _DecodeFunction<T> function) {
    ...
    final count = listDecoder.decodeLength();
    List<T> values = [];
...
extension ListDecoderUtility on ListDecoder {
  ...
  T decodeEnumeration<T>(int index, List<T> values) {
    ...
    return validateEnumeration(valueIndex, values);
  }
}
