...
class CustomerPermissions {
  ...
  CustomerPermissions({this.driverPhoneNumber});

  factory CustomerPermissions.decode(Map<String, dynamic> data) {
    if (data == null) {
      return null;
    }
...
class Customer extends Identifiable<String> implements DeletionMarking {
  ...
  factory Customer.decode(Decoder decoder) {
    ...
    decoded.contacts = decoder.decodeMapList('contacts', (data) => Contact.decode(data));
    decoded.priceType = decoder.decodeEnumeration('priceType', PriceType.values);
    decoded.unloadingPoints = decoder.decodeObjectList('unloadingPoints', (Decoder decoder) => UnloadingPoint.decode(decoder))?.excludeDeleted();
    decoded.permissions = CustomerPermissions.decode(decoder.decodeMap('permissions'));
  ...
  void encode(Encoder encoder) {
    ...
    encoder.encodeList('contacts', contacts, (ListEncoder listEncoder, Contact contact) => listEncoder.addMap(contact.encode()));
    encoder.encodeNumber('priceType', priceType.index);
    encoder.encodeList('attachedDocuments', attachedDocuments, (ListEncoder listEncoder, RemoteFile file) => listEncoder.addFile(file));
  }
}
