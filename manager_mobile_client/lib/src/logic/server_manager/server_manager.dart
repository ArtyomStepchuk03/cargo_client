...
class ServerManager {
  ...
  Future<void> configure(String companyName) async {
    ...
    final companyServersInformation = await companyServerConfigurationSource.get();
    final companyServerInformation = companyServersInformation.firstWhere((information) => information.name == companyName, orElse: () => null);
    if (companyServerInformation == null) {
      return;
    }
    _setServer(companyServerInformation.configuration);
    await storage.setServerConfiguration(companyServerInformation.configuration);
  }
  ...
  factory ServerManager.standard() => ServerManager(ServerManagerStorage(), CompanyServerConfigurationSource());
  ...
  void _setServer(parse.ServerConfiguration serverConfiguration) {
    _server = parse.ServerConfigurationUtility.fromConfiguration(serverConfiguration);
    _liveQueryManager = parse.LiveQueryManager(parse.LiveQueryConfigurationFactory.fromServer(serverConfiguration));
  }
