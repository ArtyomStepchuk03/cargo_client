...
abstract class InternalLiveQuerySubscription implements OpaqueLiveQuerySubscription {
  int get requestId;
...
class LiveQuerySubscriptionImplementation<T> implements LiveQuerySubscription<T>, InternalLiveQuerySubscription {
  final int requestId;
  ...
  LiveQuerySubscriptionImplementation({this.requestId, this.className, this.where, this.fields, this.initializer});
  ...
  void callOnCreate(Map<String, dynamic> objectData) {
    print('LiveQuery: On create: ${objectData['className']} ${objectData['objectId']}.');
    final object = initializer(Decoder(objectData));
    if (onCreate != null) onCreate(object);
    if (onAdd != null) onAdd(object);
  }
  void callOnEnter(Map<String, dynamic> objectData) {
    print('LiveQuery: On enter: ${objectData['className']} ${objectData['objectId']}.');
    final object = initializer(Decoder(objectData));
    if (onEnter != null) onEnter(object);
    if (onAdd != null) onAdd(object);
  }
  void callOnUpdate(Map<String, dynamic> objectData) {
    print('LiveQuery: On update: ${objectData['className']} ${objectData['objectId']}.');
    if (onUpdate == null) return;
    onUpdate(initializer(Decoder(objectData)));
  }
  void callOnLeave(Map<String, dynamic> objectData) {
    print('LiveQuery: On leave: ${objectData['className']} ${objectData['objectId']}.');
    final object = initializer(Decoder(objectData));
    if (onLeave != null) onLeave(object);
    if (onRemove != null) onRemove(object);
  }
  void callOnDelete(Map<String, dynamic> objectData) {
    print('LiveQuery: On delete: ${objectData['className']} ${objectData['objectId']}.');
    final object = initializer(Decoder(objectData));
