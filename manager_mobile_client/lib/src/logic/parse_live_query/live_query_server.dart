...
class LiveQueryServer {
  ...
  void subscribe(int requestId, String className, Map<String, dynamic> where, [List<String> fields]) {
  ...
  void _handleEvent(dynamic event) {
    ...
    switch (operation) {
      case 'connected': connectionDelegate?.onConnect(); break;
      case 'error': connectionDelegate?.onError(message.error); break;

      case 'subscribed': subscriptionDelegate?.onSubscribe(message.requestId); break;
      case 'unsubscribed': subscriptionDelegate?.onUnsubscribe(message.requestId); break;

      case 'create': subscriptionDelegate?.onCreate(message.requestId, message.object); break;
      case 'enter': subscriptionDelegate?.onEnter(message.requestId, message.object); break;
      case 'update': subscriptionDelegate?.onUpdate(message.requestId, message.object); break;
      case 'leave': subscriptionDelegate?.onLeave(message.requestId, message.object); break;
      case 'delete': subscriptionDelegate?.onDelete(message.requestId, message.object); break;
    }
  }
}
