...
class DistanceServerAPI {
  ...
  Future<num> get(LoadingPoint loadingPoint, UnloadingPoint unloadingPoint) async {
    final builder = parse.QueryBuilder(Distance.className);
    builder.equalToObject('loadingPoint', LoadingPoint.className, loadingPoint.id);
    builder.equalToObject('unloadingPoint', UnloadingPoint.className, unloadingPoint.id);
