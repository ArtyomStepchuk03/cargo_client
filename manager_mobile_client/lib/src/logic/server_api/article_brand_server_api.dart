...
class ArticleBrandServerAPI implements SkipPagedDataSource<ArticleBrand> {
  ...
  Future<List<ArticleBrand>> list(int skip, int limit, [ArticleType type]) async {
    ...
    return results.map((json) => ArticleBrand.decode(Decoder(json))).where((decoded) => decoded != null).toList();
  }
