...
class TransportUnitServerAPI {
  ...
  Future<List<TransportUnit>> list(TransportUnitStatus status, int skip, int limit, [Carrier carrier]) async {
    ...
    return results.map((json) => TransportUnit.decode(Decoder(json))).where((decoded) => decoded != null).toList();
  }

  Future<List<TransportUnit>> listForMap(TransportUnitStatus status, [Carrier carrier]) async {
    final builder = parse.QueryBuilder(TransportUnit.className);
    builder.equalTo('disbanded', false);
  ...
  Future<TransportUnit> create(Driver driver, Vehicle vehicle, Trailer trailer) async {
    ...
    final result = await callCloudFunction(serverManager.server, 'Dispatcher_makeTransportUnit', parameters);
    final id = result['id'];
    if (id is! String) {
      throw InvalidResponseException();
    }
    final data = await parse.getById(serverManager.server, TransportUnit.className, id, include: _getBaseIncludes());
  ...
  Future<void> fetch(TransportUnit transportUnit) async {
    final data = await parse.getById(serverManager.server, TransportUnit.className, transportUnit.id, include: _getBaseIncludes());
  ...
  Future<void> disband(TransportUnit transportUnit) async {
    ...
    await callCloudFunction(serverManager.server, 'Dispatcher_disbandTransportUnit', parameters);
  }

  parse.LiveQuerySubscription<TransportUnit> subscribeToChanges(TransportUnit transportUnit) =>
    serverManager.liveQueryManager.subscribeToObjectChanges(TransportUnit.className, transportUnit.id, (decoder) => TransportUnit.decode(decoder));
  void unsubscribe(parse.LiveQuerySubscription<TransportUnit> subscription) => serverManager.liveQueryManager.unsubscribe(subscription);
