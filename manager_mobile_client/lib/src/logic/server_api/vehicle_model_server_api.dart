...
class VehicleModelServerAPI implements SkipPagedDataSource<VehicleModel> {
  ...
  Future<List<VehicleModel>> list(int skip, int limit, [VehicleBrand brand]) async {
    ...
    return results.map((json) => VehicleModel.decode(Decoder(json))).where((decoded) => decoded != null).toList();
  }
