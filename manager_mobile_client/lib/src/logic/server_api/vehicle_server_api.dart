...
class VehicleServerAPI {
  ...
  Future<List<Vehicle>> listFree(int skip, int limit, Carrier carrier) async {
    final builder = parse.QueryBuilder(Vehicle.className);
    builder.equalTo('deleted', false);
    ...
    return results.map((json) => Vehicle.decode(Decoder(json))).where((decoded) => decoded != null).toList();
  }
  ...
  Future<void> create(Vehicle vehicle) async {
    ...
    final id = await parse.create(serverManager.server, Vehicle.className, data);
    vehicle.id = id;
  }
}
