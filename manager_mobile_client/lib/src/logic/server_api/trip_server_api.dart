...
class TripServerAPI {
  ...
  Future<List<Waypoint>> listWaypoints(Trip trip) async {
    ...
    return results.map((json) => Waypoint.decode(Decoder(json))).where((decoded) => decoded != null).toList();
  }
  ...
  Future<void> ignoreCoordinateMismatch(TripHistoryRecord historyRecord) async {
    ...
    await parse.update(serverManager.server, TripHistoryRecord.className, historyRecord.id, data);
    historyRecord.ignoreWrongCoordinate = true;
  }

  parse.LiveQuerySubscription<Trip> subscribeToChanges(Trip trip) => serverManager.liveQueryManager.subscribeToObjectChanges(Trip.className, trip.id, (decoder) => Trip.decode(decoder));
  void unsubscribe(parse.LiveQuerySubscription<Trip> subscription) => serverManager.liveQueryManager.unsubscribe(subscription);
}
