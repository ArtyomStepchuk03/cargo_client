...
class MapDataSource<T, R> implements DataSource<R> {
  ...
  Future<DataPortion<R>> loadPortion(dynamic token, int suggestedLimit) async {
    ...
    return DataPortion(portion.items.map(mapFunction).toList(), nextPortionToken: portion.nextPortionToken, finished: portion.finished);
  }
  ...
  @override
  int get hashCode => hashValues(base, mapFunction);
}
