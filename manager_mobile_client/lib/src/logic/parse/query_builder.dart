...
class QueryBuilder {
  QueryBuilder(String className) : this._internal(getClassRelativeUrl(className), className);
  QueryBuilder.users() : this._internal(userRelativeUrl, userClassName);

  void equalTo(String key, dynamic value) => _equalTo(key, value);
  void equalToObject(String key, String className, String id) => _equalTo(key, jsonForPointer(className, id));

  void equalToUserObject(String key, String id) => equalToObject(key, userClassName, id);

  void lessThan(String key, num value) => _addConstraint(key, '\$lt', value);
  void lessThanOrEqualTo(String key, num value) => _addConstraint(key, '\$lte', value);
  void greaterThan(String key, num value) => _addConstraint(key, '\$gt', value);
  void greaterThanOrEqualTo(String key, num value) => _addConstraint(key, '\$gte', value);

  void lessThanDate(String key, DateTime value) => _addConstraint(key, '\$lt', jsonFromDate(value));
  void lessThanOrEqualToDate(String key, DateTime value) => _addConstraint(key, '\$lte', jsonFromDate(value));
  void greaterThanDate(String key, DateTime value) => _addConstraint(key, '\$gt', jsonFromDate(value));
  void greaterThanOrEqualToDate(String key, DateTime value) => _addConstraint(key, '\$gte', jsonFromDate(value));

  void containedIn(String key, List<dynamic> values) => _addConstraint(key, '\$in', values);
  void notContainedIn(String key, List<dynamic> values) => _addConstraint(key, '\$nin', values);

  void exists(String key) => _exists(key, true);
  void doesNotExist(String key) => _exists(key, false);

  void matchesQuery(String key, QueryBuilder other) => _addConstraint(key, '\$inQuery', {
    'where': other._where,
    'className': other._className
  });
  ...
  Future<List<Map<String, dynamic>>> find(Server server) async {
    ...
    final results = await url_requests.find(
      server,
      _relativeUrl,
      where: encodedWhere, include: _include, skip: _skip, limit: _limit, order: _order,
    );
  ...
  Future<Map<String, dynamic>> findFirst(Server server) async {
    ...
    final results = await url_requests.find(
      server,
      _relativeUrl,
      where: encodedWhere, include: _include, skip: _skip, limit: 1, order: _order,
    );
    if (results.length == 0) {
      return null;
    }
  ...
  QueryBuilder._internal(this._relativeUrl, this._className) : _where = {}, _include = [], _skip = 0, _limit = 100, _order = [];

  void _equalTo(String key, dynamic value) => _where[key] = value;

  void _exists(String key, bool value) => _addConstraint(key, '\$exists', value);

  void _addConstraint(String key, String constraintKey, dynamic constraintValue) {
