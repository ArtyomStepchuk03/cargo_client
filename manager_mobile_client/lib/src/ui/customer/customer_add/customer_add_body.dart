...
class CustomerAddBodyState extends State<CustomerAddBody> {
  CustomerCreateInformation validate() {
    ...
    if (_contactNameKey.currentState.value != null && _contactNameKey.currentState.value.isNotEmpty) {
      createInformation.contactName = _contactNameKey.currentState.value;
    }
    if (_contactPhoneNumberKey.currentState.value != null && _contactPhoneNumberKey.currentState.value.isNotEmpty) {
      createInformation.contactPhoneNumber = _contactPhoneNumberKey.currentState.value;
    }
    ...
    createInformation.placesSearchResult = _addressKey.currentState.placesSearchResult;
  ...
  Widget _buildMainGroup(DependencyState dependencyState) {
    return buildFormGroup([
      buildFormRow(Icons.assignment_ind,
        _buildTextFormField(_nameKey, strings.name, RequiredValidator())
      ),
      buildFormRow(Icons.location_on,
        _buildAddressFormField(dependencyState.location.placesService)
      ),
      buildContactNameFormRow(key: _contactNameKey),
      buildContactPhoneNumberFormRow(key: _contactPhoneNumberKey),
      buildFormRow(Icons.account_balance_wallet,
        _buildPriceTypeFormField()
      ),
    ]);
  }
  ...
  Widget _buildAddressFormField(PlacesService placesService) {
    return AddressFormField(
      ...
      label: strings.unloadingAddress,
  ...
  Widget _buildPriceTypeFormField() {
    return EnumerationFormField<PriceType>(
      ...
      label: strings.priceTypeCustomer,
      validator: RequiredValidator(),
    );
  }

  Widget _buildTextFormField(Key key, String label, [FormFieldValidator<String> validator]) {
