...
class CustomerDataSource implements SkipPagedDataSource<Customer> {
  ...
  CustomerDataSource({this.serverAPI, this.manager, this.includeDeleted = false});

  Future<List<Customer>> list(int skip, int limit) async {
    return await serverAPI.list(skip, limit, manager: manager, includeDeleted: includeDeleted);
  }

  @override
  bool operator==(dynamic other) {
    if (other is! CustomerDataSource) {
      return false;
    }
    return true;
  }
