...
import 'transport_unit_add/transport_unit_add_widget.dart';
import 'transport_unit_add/transport_unit_add_strings.dart' as transport_unit_add_strings;
import 'transport_unit_list_body.dart';
import 'transport_unit_map_body.dart';
...
class TransportUnitTabWidget extends StatefulWidget {
  ...
  TransportUnitTabWidget({this.user, this.selecting = false, this.confirmButtonTitle, this.onConfirm, this.drawer, this.containerBuilder});

  factory TransportUnitTabWidget.main({BuildContext context, Drawer drawer, TransitionBuilder containerBuilder}) {
    final authorizationState = AuthorizationWidget.of(context);
    return TransportUnitTabWidget(user: authorizationState.user, drawer: drawer, containerBuilder: containerBuilder);
  }
...
class TransportUnitTabState extends State<TransportUnitTabWidget> with SingleTickerProviderStateMixin {
  @override
  void initState() {
    super.initState();
  ...
  CheckedPopupMenuItem<FilterValue<TransportUnitStatus>> _buildPopupMenuItem(TransportUnitStatus status) {
    return CheckedPopupMenuItem<FilterValue<TransportUnitStatus>>(
      ...
      child: Text(status != null ? formatTransportUnitStatus(status) : strings.all),
    );
  }
    ...
    return IconButton(
      ...
      onPressed: () {
        ...
        showTransportUnitSearch(
          ...
          onTap: widget.selecting ? (TransportUnit transportUnit) {
          ...
          expandedBuilder: !widget.selecting ? (BuildContext context, TransportUnit item) => _buildListBodyExpandedWidget(
            context: context,
            onDisband: () async {
            ...
            onShowOnMap: item.coordinate != null ? () {
              ...
              if (_tabController.index == 0) {
                ...
                _mapBodyKey.currentState.select(item, centerInView: true);
              }
            } : null,
          ) : null,
        );
  ...
  PopupMenuButton _buildMoreMenuButton(BuildContext context) {
    if (!widget.user.canAddTransportUnits()) {
      return null;
    }

    var items = <PopupMenuItem<GestureTapCallback>>[];
    items.add(PopupMenuItem<GestureTapCallback>(value: () => _showAddWidget(context), child: Text(transport_unit_add_strings.title)));
  ...
  void _showAddWidget(BuildContext context) async {
    final transportUnit = await Navigator.push(context, MaterialPageRoute(
      builder: (BuildContext context) => TransportUnitAddWidget(user: widget.user),
  ...
  Widget _buildListBody() {
    return TransportUnitListBody(
      ...
      status: _status,
      carrier: widget.user.carrier,
      expandedBuilder: (BuildContext context, TransportUnit item) => _buildListBodyExpandedWidget(
        context: context,
        onDisband: () async {
          bool disbanded = await _disband(context, item);
          if (!disbanded) {
    ...
    return TransportUnitMapBody(
      ...
      onCreated: () {
        if (_transportUnitToShowOnMap != null) {
          _mapBodyKey.currentState.select(_transportUnitToShowOnMap, centerInView: true);
  ...
  Widget _buildListBodyExpandedWidget({BuildContext context, VoidCallback onDisband, VoidCallback onShowOnMap}) {
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Wrap(
            ...
            spacing: 10,
            children: [
              _buildCardButton(child: Text(strings.disband), color: Colors.red, onPressed: onDisband),
              _buildCardButton(child: Text(strings.showOnMap), color: Theme.of(context).primaryColor, onPressed: onShowOnMap),
            ],
          ),
        )
      ],
  ...
  Widget _buildCardButton({VoidCallback onPressed, Color color, Widget child}) {
  ...
  Future<bool> _disband(BuildContext context, TransportUnit transportUnit) async {
    final confirmed = await showQuestionDialog(context, strings.confirmDisband);
    if (!confirmed) {
      return false;
    }
  ...
  List<Tab> _buildTabs() {
    return [
      Tab(text: strings.list),
      Tab(text: strings.map),
    ];
  }
