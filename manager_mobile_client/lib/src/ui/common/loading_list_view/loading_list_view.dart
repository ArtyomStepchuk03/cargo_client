...
import 'package:manager_mobile_client/src/ui/common/loading_list_view/loading_list_view_status.dart';
import 'loading_list_view_status.dart';
...
typedef LoadingListViewBuilder<T> = Widget Function(BuildContext context, T object);
...
class LoadingListViewState<T> extends State<LoadingListView<T>> {
  ...
  Widget _buildFooterWidget(Widget child) {
    if (child == null) {
      return buildFloatingActionButtonSpacer(widget.insetForFloatingActionButton);
    }
  ...
  Future<void> _loadPortion(DataSource<T> dataSource, {bool reload = false}) async {
    try {
      _loading = true;
