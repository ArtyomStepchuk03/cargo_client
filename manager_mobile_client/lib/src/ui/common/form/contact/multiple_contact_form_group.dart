...
class MultipleContactFormGroupState extends State<MultipleContactFormGroup> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.contacts.isEmpty) {
      children.add(buildContactNameFormRow(enabled: false));
      children.add(buildContactPhoneNumberFormRow(enabled: false));
    } else {
      final firstContact = widget.contacts.first;
      children.add(buildContactNameFormRow(initialValue: firstContact.name, enabled: false));
      children.add(buildContactPhoneNumberFormRow(initialValue: firstContact.phoneNumber, enabled: false));
      ...
      if (widget.contacts.length > 1) {
        if (_expanded) {
          for (var counter = 1; counter < widget.contacts.length; ++counter) {
            ...
            children.add(buildContactNameFormRow(initialValue: contact.name, enabled: false));
            children.add(buildContactPhoneNumberFormRow(initialValue: contact.phoneNumber, enabled: false));
          }
        } else {
