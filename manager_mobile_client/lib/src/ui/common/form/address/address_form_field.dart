...
import 'address_input_body.dart';
...
class AddressFormField extends FormField<String> {
  ...
  AddressFormField({
    ...
    validator: validator,
    enabled: enabled,
    builder: (FormFieldState<String> state) => _buildForState(state, label, enabled),
  );

  static Widget _buildForState(AddressFormFieldState state, String label, bool enabled) {
    if (state.loading) {
  ...
  static void _showInput(AddressFormFieldState state) async {
    final inputResult = await showCustomSearch(
      ...
      actionBuilder: state.widget.allowCustomAddress ? (BuildContext context, String query) {
        return IconButton(
          icon: Icon(Icons.check),
          onPressed: () => Navigator.pop(context, AddressInputResult(query)),
        );
      } : null,
    );

    if (inputResult != null) {
      state.setValueWithPlacesSearchResult(inputResult.address, inputResult.placesSearchResult);
...
class AddressFormFieldState extends ScrollableFormFieldState<String> {
  ...
  void setValueWithPlacesSearchResult(String newValue, PlacesSearchResult placesSearchResult) {
  ...
  @override
  AddressFormField get widget => super.widget;
