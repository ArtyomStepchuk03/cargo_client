...
import 'package:manager_mobile_client/src/ui/common/form/scrollable_form.dart';
...
typedef LoadingListFormFieldAddCallback<T> = Future<T> Function(BuildContext context);
...
class LoadingListFormField<T> extends FormField<T> {
  ...
  static Widget _buildForState<T>(LoadingListFormFieldState<T> state, String label, bool enabled) {
    if (state.fetching) {
    ...
    final textField = buildCustomNoneditableTextField(
      ...
      enabledBorderColor: state.widget.noteText != null ? state.widget.noteColor : null,
      disabledBorderColor: state.widget.noteText != null ? state.widget.noteColor : null,
      label: label,
  ...
  static void _showSelect<T>(LoadingListFormFieldState<T> state) async {
    ...
    T newValue = await showCustomSearch<T>(
      ...
      builder: (BuildContext context, String query) {
        final filterPredicate = query != null ? SearchFilterPredicate<T>(state.widget.searchPredicate, query) : null;
      ...
      floatingActionButtonBuilder: (BuildContext context, String query) => _buildAddButton(state),
    );
...
class LoadingListFormFieldState<T> extends ScrollableFormFieldState<T> {
  ...
  void didUpdateWidget(LoadingListFormField<T> oldWidget) {
    ...
    if (widget.dataSource != oldWidget.dataSource && widget.dataSource != null) {
      _fetchIfNeeded();
    }
  }

  @override
  LoadingListFormField<T> get widget => super.widget;

  bool _fetching;

  void _fetchIfNeeded() {
    if (!_fetching && widget.enabled && value == null && widget.fetchInitialValue) {
