...
import '../app_bar.dart';
import 'search_strings.dart' as strings;

typedef SearchWidgetBuilder = Widget Function(BuildContext context, String query);
...
Future<T> showCustomSearch<T>({
  ...
  return await Navigator.of(context).push<T>(MaterialPageRoute(
...
class _SearchWidget extends StatefulWidget {
  ...
  _SearchWidget({this.initialQuery, this.builder, this.actionBuilder, this.floatingActionButtonBuilder});
...
class _SearchState extends State<_SearchWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: widget.floatingActionButtonBuilder != null ? widget.floatingActionButtonBuilder(context, query) : null,
    );
  }
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        border: InputBorder.none,
          suffixIcon: _textEditingController.text.isNotEmpty ? _buildCircleIconButton(icon: Icons.clear, onPressed: _textEditingController.clear) : null
        ),
        style: textStyle,
      );
  }
  ...
  Widget _buildBody(String query) {
    ...
    if (query.isNotEmpty && query.length < 3) {
      children.add(_buildPlaceholder(strings.queryTooShort));
    }
