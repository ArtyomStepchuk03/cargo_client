...
class UngroupedReservationListBodyState extends State<UngroupedReservationListBody> implements ReservationListBodyState {
  ...
  Widget build(BuildContext context) {
    if (_reservations.isEmpty) {
      return FullscreenPlaceholder(icon: Icons.table_view, text: strings.noEntries);
    }
    return ListView(
      children: _reservations.map((reservation) => _buildCell(context, reservation)).toList(),
    );
  }
  ...
  BoxBorder _buildBorder(BuildContext context) => Border(bottom: BorderSide(color: Theme.of(context).dividerColor));
}
