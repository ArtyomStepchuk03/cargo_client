...
import 'reservation_details/reservation_details_widget.dart';
import 'reservation_add/reservation_add_widget.dart';
...
class SupplierGroupReservationListBody extends StatefulWidget {
  ...
  final Map<Tuple3<ArticleBrand, Supplier, LoadingPoint>, num> purchaseTariffMap;
  final User user;
  final DateTime date;

  SupplierGroupReservationListBody(this.reservationMap, {this.reservationMapDayBefore, this.configuration, this.purchaseTariffMap, this.user, this.date});

  factory SupplierGroupReservationListBody.fromList(List<Order> reservations, {
    ...
    return SupplierGroupReservationListBody(reservationMap, reservationMapDayBefore: reservationMapDayBefore, configuration: configuration, purchaseTariffMap: purchaseTariffMap, user: user, date: date);
  }

  @override
  State<StatefulWidget> createState() => SupplierGroupReservationListBodyState();
}

class SupplierGroupReservationListBodyState extends State<SupplierGroupReservationListBody> implements ReservationListBodyState {
  void addReservation(Order reservation) {
    if (reservation.unloadingBeginDate.toLocal().beginningOfDay != widget.date) {
      return;
    }
    setState(() {
      _reservationMap.addReservation(reservation, toBeginning: true);
      if (_reservationMap.getCount(reservation.supplier, reservation.articleBrand) == 1) {
        _expand(reservation);
      }
    });
  }
  ...
  Widget _buildSupplierList(BuildContext context) {
    return ExpansionPanelList(
      ...
      elevation: 1,
      expansionCallback: (int index, bool isExpanded) => setState(() => _supplierExpansionState[widget.configuration.suppliers[index]] = !isExpanded),
      children: widget.configuration.suppliers.map((supplier) => _buildSupplierTile(context, supplier)).toList(),
    );
  }

  ExpansionPanel _buildSupplierTile(BuildContext context, Supplier supplier) {
    final reservationMap = _reservationMap[supplier] ?? {};
    final reservationMapDayBefore = widget.reservationMapDayBefore[supplier] ?? {};

    if (_supplierExpansionState[supplier] == null) {
      _supplierExpansionState[supplier] = reservationMap.values.any((reservations) => reservations.isNotEmpty);
    }

    return ExpansionPanel(
      headerBuilder: (context, isExpanded) => ListTile(
        title: Text(short.formatSupplierSafe(supplier)),
        subtitle: buildSupplierReservationSummary(reservationMap.values.expand((reservation) => reservation).toList(), purchaseTariffMap: widget.purchaseTariffMap),
        contentPadding: EdgeInsets.only(left: 16),
        onTap: () => setState(() => _supplierExpansionState[supplier] = !_supplierExpansionState[supplier]),
      ),
      backgroundColor: CommonColors.reservationsSupplierHeader,
      body: _buildArticleBrandList(context, supplier, reservationMap, reservationMapDayBefore),
  ...
  Widget _buildArticleBrandList(
    ...
    if (supplier.articleBrands == null || supplier.articleBrands.isEmpty) {
      return buildReservationListPlaceholderCell(context: context, text: strings.noArticleBrands, border: _buildBorder(context));
    }
    return Container(
      color: Colors.white,
      child: ExpansionPanelList(
        expandedHeaderPadding: EdgeInsets.zero,
        elevation: 1,
        expansionCallback: (int index, bool isExpanded) => setState(() => _articleBrandExpansionState[Tuple2(supplier, supplier.articleBrands[index])] = !isExpanded),
        children: supplier.articleBrands.map((articleBrand) => _buildArticleBrandTile(context, supplier, articleBrand, reservationMap, reservationMapDayBefore)).toList(),
      ),
    );
  }
  ...
  ExpansionPanel _buildArticleBrandTile(
    ...
    if (_articleBrandExpansionState[Tuple2(supplier, articleBrand)] == null) {
      _articleBrandExpansionState[Tuple2(supplier, articleBrand)] = reservations.isNotEmpty;
    }

    return ExpansionPanel(
      headerBuilder: (context, isExpanded) => ListTile(
        title: Text(short.formatArticleBrandSafe(articleBrand)),
        subtitle: buildArticleReservationSummary(reservations, reservationsDayBefore: reservationsDayBefore),
        trailing: widget.user.canAddOrders() ? IconButton(
          icon: Icon(Icons.add_circle_outline),
          color: Theme.of(context).primaryColor,
          onPressed: () => _showAddWidget(context: context, supplier: supplier, articleBrand: articleBrand),
        ) : null,
        onTap: () => setState(() => _articleBrandExpansionState[Tuple2(supplier, articleBrand)] = !_articleBrandExpansionState[Tuple2(supplier, articleBrand)]),
        contentPadding: EdgeInsets.only(left: 16),
      ),
  ...
  Widget _buildReservationList(BuildContext context, List<Order> reservations) {
    if (reservations.isEmpty) {
      return buildReservationListPlaceholderCell(context: context, text: strings.noEntries, border: _buildBorder(context));
    }
    return Column(
      children: reservations.map((reservation) => _buildReservationCell(context, reservation)).toList(),
    );
  }
  ...
  Widget _buildTotalSummaryCell(BuildContext context) {
    return buildReservationListMultipleFieldCell(
      context: context,
      children: [
        buildReservationsCountWidget(_reservationMap),
        buildTotalPurchasePriceWidget(_reservationMap, widget.purchaseTariffMap),
      ],
      border: _buildBorder(context),
    );
  }
  ...
  void _showDetails(BuildContext context, Order reservation) {
    Navigator.push(context, MaterialPageRoute(
      builder: (BuildContext context) => ReservationDetailsWidget(reservation: reservation, user: widget.user, listBodyState: this),
    ));
  }

  void _showAddWidget({BuildContext context, Supplier supplier, ArticleBrand articleBrand}) async {
    ...
    final newReservation = await Navigator.push(context, MaterialPageRoute(
      builder: (BuildContext context) => ReservationAddWidget(user: widget.user, reservation: reservation),
  ...
  void _expand(Order reservation) {
    ...
    _articleBrandExpansionState[Tuple2(reservation.supplier, reservation.articleBrand)] = true;
  }

  void _subscribe() {
    _subscription = _serverAPI.subscribeToReservations(widget.user, widget.date);
  ...
  BoxBorder _buildBorder(BuildContext context) => Border(top: BorderSide(color: Theme.of(context).dividerColor));
}
