...
import 'package:manager_mobile_client/src/ui/common/date_control.dart';
...
typedef ReservationListBuilder = Widget Function(BuildContext context, ReservationListLoadResult loadResult, ReservationListSharedData sharedData, User user, DateTime date);
...
class ReservationListState extends State<ReservationListWidget> {
  ...
  Widget _buildBody() {
    ...
    return widget.builder(context, _loadStatus.result, _sharedData, _user, _date);
  }
  ...
  Future<void> _loadSharedData() async {
    ...
    try {
      ...
      final configuration = dependencyState.network.configurationLoader.configuration;
      final purchaseTariffs = await dependencyState.network.serverAPI.purchaseTariffs.list();
      ...
      setState(() => _sharedData = ReservationListSharedData(configuration: configuration, purchaseTariffMap: purchaseTariffMap));
    } catch (exception) {
      if (!mounted) {
        return;
      }
  ...
  Future<void> _load(DateTime date) async {
    ...
    try {
      ...
      final reservationsDayBefore = await _serverAPI.listReservations(_user, date.subtract(Duration(days: 1)));
      status = DataLoadStatus.succeeded(ReservationListLoadResult(reservations, reservationsDayBefore: reservationsDayBefore));
    } catch (exception) {
