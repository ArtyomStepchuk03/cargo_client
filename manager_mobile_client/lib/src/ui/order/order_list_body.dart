...
class OrderListBodyState extends State<OrderListBody> {
  ...
  void removeOrder(Order order) => _listViewKey.currentState.removeItem(order);
  void updateOrder(Order order) => _listViewKey.currentState.updateItem(order);
  ...
  Widget build(BuildContext context) {
    return LoadingListView(
      key: _listViewKey,
      dataSource: makeOrderDataSource(serverAPI: _serverAPI, user: widget.user, sort: widget.sort, filter: widget.filter),
      filterPredicate: widget.filterPredicate,
      builder: (BuildContext context, Order order) {
        return _buildCard(context, order, DependencyHolder.of(context).network.configurationLoader.configuration);
      },
  ...
  Widget _buildCard(BuildContext context, Order order, Configuration configuration) {
  ...
  void _showDetails(BuildContext context, Order order) {
    Navigator.push(context, MaterialPageRoute(
      builder: (BuildContext context) => OrderDetailsWidget(order: order, user: widget.user, listBodyState: this),
    ));
  }
