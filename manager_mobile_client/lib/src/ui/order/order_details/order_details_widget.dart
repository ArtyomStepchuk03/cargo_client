...
import 'package:manager_mobile_client/src/ui/common/speed_dial_button.dart';
...
import 'package:manager_mobile_client/src/ui/order/trip_problem_list/trip_problem_list_widget.dart';
import 'package:manager_mobile_client/src/ui/order/order_action_check.dart';
import 'package:manager_mobile_client/src/ui/order/order_list_body.dart';
...
import 'package:manager_mobile_client/src/ui/order/trip_problem_list/trip_problem_list_strings.dart' as trip_problem_strings;
import 'package:manager_mobile_client/src/ui/order/order_map/order_map_widget.dart';
import 'package:manager_mobile_client/src/ui/order/order_finish/order_finish_strings.dart' as order_finish_strings;
import 'package:manager_mobile_client/src/ui/order/order_finish/order_finish_widget.dart';
import 'package:manager_mobile_client/src/ui/order/order_send/transport_unit_send_widget.dart';
import 'package:manager_mobile_client/src/ui/order/order_send/carrier_send_widget.dart';
import '../../common/app_bar.dart';
import 'order_details_main_body.dart';
...
class OrderDetailsState extends State<OrderDetailsWidget> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  ...
  void didChangeDependencies() {
    ...
    if (_progressNotifier == null) {
      ...
      _progressNotifier = OrderProgressNotifier(serverAPI.orders, serverAPI.offers, serverAPI.trips);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: buildAppBar(
        title: Text(formatWithNumberSafe(strings.title, widget.order.number)),
  ...
  TabBar _buildTabBar() {
    ...
    return TabBar(
      controller: _tabController,
      tabs: [
        Tab(text: strings.mainTitle),
        Tab(text: strings.progressTitle),
      ],
    );
  }
  ...
  List<Widget> _buildActions(BuildContext context) {
    ...
    return [
      ...
      IconButton(icon: Icon(Icons.mode_of_travel), onPressed: () => _showMap(context)),
    ...
    return PopupMenuButton<GestureTapCallback>(
      ...
      itemBuilder: (BuildContext context) => [
        ...
        if (_shouldShowTripProblems(widget.order))
          PopupMenuItem<GestureTapCallback>(value: () => _showTripProblems(context), child: Text(trip_problem_strings.title)),
        if (_canCancelCarrierSend(widget.order))
          PopupMenuItem<GestureTapCallback>(value: () => _cancelCarrierSend(context), child: Text(strings.cancelCarrierSend)),
        if (widget.order.canDeclineCarrierOffer(widget.user))
        ...
        if (_canFinishOrderManually(widget.order))
          PopupMenuItem<GestureTapCallback>(value: () => _showFinishDialog(context), child: Text(order_finish_strings.title)),
        if (_canCancelOrderByCustomer(widget.order))
          PopupMenuItem<GestureTapCallback>(value: () => _cancelByCustomer(context), child: Text(strings.cancel))
        else if (_canCancelOrder(widget.order))
          PopupMenuItem<GestureTapCallback>(value: () => _cancel(context), child: Text(strings.cancel)),
        ...
        if (widget.user.role == Role.customer)
          PopupMenuItem<GestureTapCallback>(value: () => _contactManager(context), child: Text(strings.contactManager))
      ],
  ...
  Widget _buildCurrentFloatingActionButton(context) {
    ...
    if (widget.order.canAcceptCarrierOffer(widget.user)) {
      return buildFloatingActionButtonContainer(
        child: FloatingActionButton.extended(
          icon: Container(),
          label: Text(strings.confirmUppercase),
          backgroundColor: Theme.of(context).primaryColor,
          onPressed: () => _accept(context),
        ),
      );
    }
    if (!widget.user.canAssignCarriers()) {
      return buildFloatingActionButtonContainer(
        child: FloatingActionButton.extended(
          icon: Container(),
          label: Text(strings.sendFloatingActionButton),
          backgroundColor: Theme.of(context).primaryColor,
    ...
    return SpeedDialButton(
      ...
      items: [
        SpeedDialButtonItem(
          label: Text(strings.sendToTransportUnits),
          icon: Icon(Icons.local_shipping),
          onPressed: () => _showTransportUnitSendDialog(context),
        ),
        SpeedDialButtonItem(
          label: Text(strings.sendToCarriers),
  ...
  void _showTripProblems(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(
      builder: (BuildContext context) => TripProblemListWidget(widget.order),
    ));
  }
  ...
  void _showAddWidget(BuildContext context) async {
    ...
    final newOrder = await Navigator.pushReplacement(context, MaterialPageRoute(
      builder: (BuildContext context) => OrderAddWidget.clone(user: widget.user, order: widget.order),
  ...
  void _showTransportUnitSendDialog(BuildContext context) async {
    final sentOrder = await Navigator.push(context, MaterialPageRoute(
      builder: (BuildContext context) => buildTransportUnitSendWidget(context, widget.order, widget.user),
      fullscreenDialog: true,
    ));
    if (sentOrder != null) {
  ...
  void _showCarrierSendDialog(BuildContext context) async {
    final sentOrder = await Navigator.push(context, MaterialPageRoute(
      builder: (BuildContext context) => buildCarrierSendWidget(context, widget.order, widget.user),
      fullscreenDialog: true,
    ));
    if (sentOrder != null) {
      _tabController.index = 0;
  ...
  void _cancelCarrierSend(BuildContext context) async {
    final confirmed = await showQuestionDialog(context, strings.confirmCancelCarrierSend);
    if (confirmed) {
      final serverAPI = DependencyHolder.of(context).network.serverAPI.orders;
  ...
  void _showFinishDialog(BuildContext context) async {
    final finishedOrder = await Navigator.push(context, MaterialPageRoute(
  ...
  void _cancel(BuildContext context) async {
    final confirmed = await showQuestionDialog(context, strings.confirmCancel);
    if (confirmed) {
      showDefaultActivityDialog(context);
      final dependencyState = DependencyHolder.of(context);
      final serverAPI = dependencyState.network.serverAPI.orders;
      try {
        ...
        if (exception.error == ServerError.invalidTripStageForCancel) {
          showErrorDialog(context, strings.cannotCancel);
          return;
        }
        showDefaultErrorDialog(context);
      } on Exception {
    ...
    if (confirmed) {
      ...
      try {
        ...
        if (exception.error == ServerError.cannotCancelByCustomer) {
          showErrorDialog(context, strings.cannotCancelByCustomer);
          return;
        }
        showDefaultErrorDialog(context);
      } on Exception {
  ...
  void _contactManager(BuildContext context) async {
    ...
    if (phoneNumber == null) {
      await showInformationDialog(context, strings.cannotContactManager);
      return;
    }
  ...
  bool _needsFloatingActionButton() {
    return !_editing && (widget.order.canAcceptCarrierOffer(widget.user) || _canSendOrder(widget.order));
  }
  ...
  bool _canEditOrder(Order order) {
    ...
    if (widget.user.role == Role.dispatcher && !widget.order.isActionsAllowedForDispatcher(widget.user)) {
      return false;
    }
    if (order.deleted) {
      return false;
  ...
  bool _canDeleteOrder(Order order) {
    ...
    if (widget.user.role == Role.dispatcher && (order.author == null || order.author.id != widget.user.id)) {
      return false;
    }
    if (order.deleted) {
      return false;
