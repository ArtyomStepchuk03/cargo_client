...
import 'package:manager_mobile_client/src/ui/common/fullscreen_activity_overlay.dart';
import 'package:manager_mobile_client/src/ui/common/multiline_detail_window.dart';
...
class OrderMapState extends State<OrderMapWidget> {
  ...
  void dispose() {
    ...
    if (_subscription != null) _transportUnitServerAPI.unsubscribe(_subscription);
    super.dispose();
  }
  ...
  Marker _buildEntranceMarker(String identifier, BitmapDescriptor icon, Entrance entrance) {
    return Marker(
      ...
      onTap: () {
        showMultilineDetailWindow(context: context, lines: [
          ...
          strings.address(textOrEmpty(entrance.address)),
        ]);
      },
    );
  }
  ...
  Marker _buildLoadingEntranceMarker(Entrance entrance) {
    return _buildEntranceMarker('LoadingEntrance', _imageList.loadingEntranceIcon, entrance);
  }

  Marker _buildUnloadingEntranceMarker(Entrance entrance) {
    return _buildEntranceMarker('UnloadingEntrance', _imageList.unloadingEntranceIcon, entrance);
  }
  ...
  Marker _buildUnloadedMarker(TransportUnit transportUnit, TripHistoryRecord inUnloadingPointRecord, TripHistoryRecord unloadedRecord) {
    int timeInterval;
    if (inUnloadingPointRecord.date != null && unloadedRecord.date != null) {
      timeInterval = unloadedRecord.date.difference(inUnloadingPointRecord.date).inMilliseconds;
    }
    return Marker(
      ...
      onTap: () {
        showMultilineDetailWindow(context: context, lines: [
          strings.date(formatDateSafe(inUnloadingPointRecord.date)),
          strings.unloadingTimeInterval(formatTimeIntervalSafe(timeInterval)),
          strings.stateNumber(textOrEmpty(transportUnit?.vehicle?.number)),
          strings.address(textOrEmpty(inUnloadingPointRecord.address)),
        ]);
      },
    );
  }

  Marker _buildWaypointMarker(Trip trip, int index, Waypoint waypoint, int historyRecordIndex, BitmapDescriptor icon) {
    final lines = <String>[];
    lines.add(strings.date(formatDateSafe(waypoint.date)));

    if (historyRecordIndex != -1) {
      lines.add(strings.stage(formatTripStage(trip.historyRecords[historyRecordIndex].stage)));
    }
  ...
  Marker _buildDirectionMarker(int index, LatLng coordinate, double heading, BitmapDescriptor icon) {
    return Marker(markerId: MarkerId('Direction$index'), flat: true, position: coordinate, rotation: heading, icon: icon);
  }

  Polyline _buildPolyline(String identifier, Color color, List<LatLng> coordinates) {
    return Polyline(
  ...
  void _buildKeyMarkers() {
    if (widget.order.loadingEntrance != null && widget.order.loadingEntrance.coordinate != null) {
      _keyMarkers.add(_buildLoadingEntranceMarker(widget.order.loadingEntrance));
    }
    if (widget.order.unloadingEntrance != null && widget.order.unloadingEntrance.coordinate != null) {
      _keyMarkers.add(_buildUnloadingEntranceMarker(widget.order.unloadingEntrance));
    }

    final offer = widget.order.getAcceptedOffer();
    if (offer != null) {
      if (offer.trip != null) {
        final inUnloadingPointRecord = offer.trip.getHistoryRecord(TripStage.inUnloadingPoint);
        final unloadedRecord = offer.trip.getHistoryRecord(TripStage.unloaded);
        if (inUnloadingPointRecord != null && unloadedRecord != null && inUnloadingPointRecord.coordinate != null) {
          _keyMarkers.add(_buildUnloadedMarker(offer.transportUnit, inUnloadingPointRecord, unloadedRecord));
        }
      }
    }
  }
  ...
  void _buildTripOverlays(Trip trip, List<Waypoint> waypoints) {
    ...
    _polylines.add(_buildPolyline('LoadingTrip', Colors.blue, loadingWaypoints.map((waypoint) => waypoint.coordinate).toList()));
    _buildTripMarkers(trip, loadingWaypoints, _imageList.loadingWaypointIcon, _imageList.loadingDirectionIcon);

    if (unloadingWaypoints != null) {
      _polylines.add(_buildPolyline('UnloadingTrip', Colors.red, unloadingWaypoints.map((waypoint) => waypoint.coordinate).toList()));
      _buildTripMarkers(trip, unloadingWaypoints, _imageList.unloadingWaypointIcon, _imageList.unloadingDirectionIcon);
    }
  }

  void _buildTripMarkers(Trip trip, List<Waypoint> waypoints, BitmapDescriptor waypointIcon, BitmapDescriptor directionIcon) {
    ...
    _tripMarkers.add(_buildWaypointMarker(trip, 0, waypoints[0], historyRecordIndex, waypointIcon));
    ...
    for (int counter = 0; counter < waypoints.length - 1; ++counter) {
      ...
      historyRecordIndex = _getHistoryRecordIndex(trip, other, historyRecordIndex);
      ...
      if (waypointDistance > 400) {
        waypointDistance = 0;
        _tripMarkers.add(_buildWaypointMarker(trip, counter + 1, waypoints[counter + 1], historyRecordIndex, waypointIcon));
      }
      ...
      if (segmentLength > 10 && directionDistance > 800) {
        ...
        final coordinate = LatLng(
          one.coordinate.latitude + (other.coordinate.latitude - one.coordinate.latitude) / 2,
          one.coordinate.longitude + (other.coordinate.longitude - one.coordinate.longitude) / 2
        );

        final heading = getHeading(one.coordinate, other.coordinate);
        _tripMarkers.add(_buildDirectionMarker(counter, coordinate, heading, directionIcon));
      }
  ...
  void _handleCameraMove(CameraPosition position) async {
    ...
    if (_lastCoordinate != null && !shouldUpdateMarkers(_lastCoordinate, position.target)) {
      return;
    }
  ...
  void _loadTripData() async {
    ...
    final orderFinished = offer?.trip != null && offer.trip.stage == TripStage.unloaded;
    ...
    if (waypoints != null && waypoints.isNotEmpty) {
      final bounds = LatLngBoundsUtility.circumscribed(waypoints.map((waypoint) => waypoint.coordinate));
      _controller.animateCamera(CameraUpdate.newLatLngBounds(bounds, 20));
    } else if (_transportUnit?.coordinate != null) {
      _controller.animateCamera(CameraUpdate.newLatLngZoom(_transportUnit.coordinate, _followingZoom));
    }
  ...
  void subscribe(TransportUnit transportUnit) {
    ...
    _subscription.onUpdate = (updatedTransportUnit) async {
      ...
      if (_transportUnitMarker != null && _followTransportUnit) {
        _controller.animateCamera(CameraUpdate.newLatLngZoom(updatedTransportUnit.coordinate, _followingZoom));
      }
    };
  }
  ...
  int _getLoadedWaypointIndex(Trip trip, List<Waypoint> waypoints) {
    ...
    return waypoints.indexWhere((waypoint) => !waypoint.date.isBefore(loadedRecord.date));
  }

  int _getHistoryRecordIndex(Trip trip, Waypoint waypoint, [int startIndex = -1]) {
    if (trip.historyRecords == null) {
      return -1;
    }
    while (startIndex + 1 < trip.historyRecords.length && waypoint.date.isAfter(trip.historyRecords[startIndex + 1].date)) {
      ++startIndex;
    }
    return startIndex;
  }
  ...
  CameraPosition _getInitialCameraPosition() {
    if (widget.order.unloadingEntrance != null && widget.order.unloadingEntrance.coordinate != null) {
      return CameraPosition(target: widget.order.unloadingEntrance.coordinate, zoom: defaultCameraPosition.zoom);
    }
