...
class OrderListCard extends StatelessWidget {
    ...
    return ListCard(
      ...
      children: [
        if (user.role == Role.dispatcher)
          ListCardField(name: strings.intermediary, value: short.formatIntermediarySafe(order.intermediary)),
        if (user.role != Role.customer)
          ListCardField(name: strings.customer, value: short.formatCustomerSafe(order.customer)),
        ListCardField(name: strings.address, value: short.formatUnloadingPointSafe(order.unloadingPoint)),
        Row(children: [
          Expanded(flex: 2, child: ListCardField(name: strings.tonnage, value: formatTonnage(order.tonnage))),
          Expanded(flex: 3, child: ListCardField(name: strings.time, value: _formatUnloadingTime())),
        ]),
        Row(children: [
          Expanded(flex: 2, child: ListCardField(name: strings.orderNumber, value: numberOrEmpty(order.number))),
          Expanded(flex: 3, child: ListCardField(name: strings.unloading, value: _formatUnloadingText())),
        ]),
        if (user.role == Role.customer)
          ListCardField(name: strings.article, value: short.formatArticleBrandSafe(order.articleBrand)),
        ListCardField(name: strings.stageShort, value: formatOrderStatus(order, user, flags: OrderStatusFormatFlags.all(user.role != Role.customer)))
      ],
      onTap: onTap,
    );
  }
  ...
  String _formatUnloadingTime() {
    ...
    return format_strings.timeRange(formatTimeOnly(order.unloadingBeginDate), formatTimeOnly(order.unloadingEndDate));
  }

  String _formatUnloadingText() {
    final tripHistoryRecord = order.getAcceptedOffer()?.trip?.getHistoryRecord(TripStage.unloaded);
  ...
  Color _getCardBackgroundColor() {
    ...
    if (order.offers == null || order.offers.isEmpty) {
      if (user.role != Role.dispatcher && order.carriers != null && order.carriers.isNotEmpty) {
...
class _OrderStatusColorPalette {
  ...
  const _OrderStatusColorPalette({this.untouched, this.transferred, this.inProgress, this.finished});
