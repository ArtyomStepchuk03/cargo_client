...
class CarrierListBody extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => CarrierListBodyState();
}

class CarrierListBodyState extends State<CarrierListBody> {
  ...
  Widget build(BuildContext context) {
    return LoadingListView<Carrier>(
      dataSource: SkipPagedDataSourceAdapter(DependencyHolder.of(context).network.serverAPI.carriers),
      filterPredicate: widget.filterPredicate,
      builder: (BuildContext context, Carrier carrier) => _buildCard(context, carrier),
  ...
  Widget _buildCard(BuildContext context, Carrier carrier) {
    final children = [
      ListCardField(value: short.formatCarrierSafe(carrier)),
    ];
    if (widget.selecting) {
