...
String formatHours(int timeInterval) {
  return '${timeInterval ~/ 1000 ~/ 60 ~/ 60}';
}

String formatMinutes(int timeInterval) {
...
String formatTimeInterval(int timeInterval) {
  ...
  if (days != 0) {
    components.add('$days ${strings.days}');
  }
  if (hours != 0) {
    components.add('$hours ${strings.hours}');
  }
  if ((days == 0 && hours == 0) || minutes != 0) {
    components.add('$minutes ${strings.minutes}');
  }
...
String formatHoursSafe(int timeInterval) {
  return textOrEmpty(timeInterval != null ? formatHours(timeInterval) : null);
}

String formatMinutesSafe(int timeInterval) {
  return textOrEmpty(timeInterval != null ? formatMinutes(timeInterval) : null);
}

String formatTimeIntervalSafe(int timeInterval) {
  return textOrEmpty(timeInterval != null ? formatTimeInterval(timeInterval) : null);
}
...
String formatDateOnlySafe(DateTime date) {
  return textOrEmpty(date != null ? formatDateOnly(date) : null);
}

String formatDateOnlyShortSafe(DateTime date) {
  return textOrEmpty(date != null ? formatDateOnlyShort(date) : null);
}

String formatTimeOnlySafe(DateTime date) {
  return textOrEmpty(date != null ? formatTimeOnly(date) : null);
}

String formatDateSafe(DateTime date) {
  return textOrEmpty(date != null ? formatDate(date) : null);
}
