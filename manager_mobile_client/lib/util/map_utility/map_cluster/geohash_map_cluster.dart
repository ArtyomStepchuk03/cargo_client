...
class GeohashMapClusterItem<T> {
  ...
  factory GeohashMapClusterItem.encode(LatLng coordinate, T value) {
    return GeohashMapClusterItem._(coordinate, Geohash.encode(coordinate.latitude, coordinate.longitude, codeLength: _geohashLevelCount), value);
  }

  GeohashMapClusterItem<T> toLevel(int level) {
    return GeohashMapClusterItem._(coordinate, geohash.substring(0, level), this.value);
  }
...
extension GeohashMapCluster<T> on List<GeohashMapClusterItem<T>> {
  List<MapCluster<T>> buildClusters(double zoom) {
    ...
    while (unprocessed.length != 0) {
      ...
      final nearest = unprocessed.removeAndReturnWhere((other) => other.geohash == current.geohash);
      final bounds = LatLngBoundsUtility.circumscribed(nearest.map((item) => item.coordinate));
      final center = bounds.center;
      clusters.add(MapCluster<T>(center, nearest.map((item) => item.value).toList()));
    }
