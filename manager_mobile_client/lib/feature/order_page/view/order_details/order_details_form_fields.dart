...
Widget buildSupplierFormField(
  ...
  if (user.role == Role.dispatcher && !user.carrier.showAllSuppliers) {
    return _buildSupplierFormField(
      ...
      onChanged: onChanged,
      onRefresh: () =>
          dependencyState.caches.allowedSupplier.getCache(user.carrier).clear(),
      enabled: enabled,
    );
  }
  return _buildSupplierFormField(
...
Widget buildCustomerFormField(
  ...
  if (user.role == Role.dispatcher && !user.carrier.showAllCustomers) {
    return _buildCustomerFormField(
      ...
      onChanged: onChanged,
      onRefresh: () =>
          dependencyState.caches.allowedCustomer.getCache(user.carrier).clear(),
      enabled: enabled,
    );
  }
  return _buildCustomerFormField(
  ...
  return LoadingListFormField<Customer>(
    ...
    listViewBuilder: (BuildContext context, Customer customer) {
      return SimpleListTile(customer.name, customer.itn,
          customer.internal ? null : Icons.keyboard_arrow_right);
    },
    onChanged: onChanged,
    onRefresh: onRefresh,
