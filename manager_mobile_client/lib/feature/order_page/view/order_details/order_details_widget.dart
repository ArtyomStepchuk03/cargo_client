...
class OrderDetailsState extends State<OrderDetailsWidget>
  ...
  void _showTransportUnitSendDialog(BuildContext context) async {
    final sentOrder = await Navigator.push(
        ...
        MaterialPageRoute(
          builder: (BuildContext context) =>
              buildTransportUnitSendWidget(context, widget.order, widget.user),
          fullscreenDialog: true,
        ));
    if (sentOrder != null) {
  ...
  bool _needsFloatingActionButton() {
    return !_editing &&
        (widget.order.canAcceptCarrierOffer(widget.user) ||
  ...
  bool _canEditOrder(Order order) {
    ...
    if (widget.user.role == Role.dispatcher &&
        !widget.order.isActionsAllowedForDispatcher(widget.user)) {
      return false;
    }
    if (order.deleted) {
      return false;
